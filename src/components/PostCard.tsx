...
export function PostCard({ post, queryKey }: PostCardProps) {
  ...
  let doLikeThing = () => { // fires when clicking heart
    if (!isAuthenticated) { navigate("/auth"); return; }
    likeMutation.mutate();
  };

  let sb = post.body.length > 200 ? post.body.slice(0, 200) + "…" : post.body; // idk doing some substring
  // console.log("render", post.id); // todo remove this
